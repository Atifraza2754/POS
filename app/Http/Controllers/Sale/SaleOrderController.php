...
class SaleOrderController extends Controller
{
    ...
    //             // CREATE NEW
    //             $validatedData['grand_total'] = $validatedData['discounted_total'];
    //             $newSaleOrder = SaleOrder::create($validatedData);
    //             $request->merge(['sale_order_id' => $newSaleOrder->id]);
    ...
    //         $request->merge(['modelName' => $newSaleOrder]);

    //         // SAVE ITEMS
    //         $itemsSaved = $this->saveSaleOrderItems($request);
    //         if (!$itemsSaved['status']) {
    //             throw new \Exception($itemsSaved['message']);
    //         }

    //         // SAVE PAYMENTS
    //         $paymentsSaved = $this->saveSaleOrderPayments($request);
    //         if (!$paymentsSaved['status']) {
    //             throw new \Exception($paymentsSaved['message']);
    //         }
    ...
    public function store(SaleOrderRequest $request) : JsonResponse
    {
        try {
            ...
            if ($request->operation === 'save') {
                ...
                $saleOrder = SaleOrder::create($validatedData);

                $request->merge(['sale_order_id' => $saleOrder->id]);
            ...
            // **********************************
                ...
                $saleOrder = SaleOrder::findOrFail($validatedData['sale_order_id']);

                // STEP 1 — RESTORE OLD STOCK BEFORE UPDATING
                $oldTransactions = $saleOrder->itemTransaction;
                ...
                foreach ($oldTransactions as $old) {
                    ...
                    if ($item) {
                        ...
                        $isBox = ($old->unit_id == $item->base_unit_id);
                        $isPiece = ($old->unit_id == $item->secondary_unit_id);

                        if ($isBox) {
                            // restore boxes
                            $item->current_stock += $old->quantity;
                            $item->current_pieces_stock += ($old->quantity * $item->conversion_rate);
                        }

                        if ($isPiece) {
                            // restore pieces
                            $item->current_pieces_stock += $old->quantity;
                            $item->current_stock = $item->current_pieces_stock / $item->conversion_rate;
                        }
                ...
                // STEP 2 — NOW DELETE OLD TRANSACTIONS
                $saleOrder->itemTransaction()->delete();

                // STEP 3 — UPDATE SALE ORDER HEADER
                $saleOrder->update([
                    'party_id'      => $validatedData['party_id'],
                    'order_date'    => $validatedData['order_date'],
                    'due_date'      => $validatedData['due_date'],
            ...
            // **********************************
            // Allow saveSaleOrderItems() to access sale order
            // **********************************
            $request->merge(['modelName' => $saleOrder]);


            // **********************************
            // SAVE NEW SALE ORDER ITEMS
            // **********************************
            $itemSave = $this->saveSaleOrderItems($request);

            if (!$itemSave['status']) {
                throw new \Exception($itemSave['message']);
            }


            // **********************************
            // UPDATE STOCK BASED ON NEW VALUES
            // **********************************
            ...
            foreach ($items as $row) {
                ...
                if (!$item) {
                    throw new \Exception("Item not found (ID: {$row['item_id']}).");
                }
                ...
                $isBox = ($unitId == $item->base_unit_id);
                $isPiece = ($unitId == $item->secondary_unit_id);

                if (!$isBox && !$isPiece) {
                    throw new \Exception("Invalid unit for item: {$item->name}");
                }
                ...
                if ($isBox) {
                    ...
                    if ($item->current_stock < $qty) {
                        throw new \Exception("Not enough boxes for {$item->name}. Available: {$item->current_stock}");
                    }

                    $item->current_stock -= $qty;
                    $item->current_pieces_stock -= ($qty * $item->conversion_rate);
                }

                if ($isPiece) {

                    if ($item->current_pieces_stock < $qty) {
                        throw new \Exception("Not enough pieces for {$item->name}. Available: {$item->current_pieces_stock}");
                    }

                    $item->current_pieces_stock -= $qty;
                    $item->current_stock = $item->current_pieces_stock / $item->conversion_rate;
                }

                // MINIMUM STOCK CHECK
                if ($item->current_stock < $item->min_stock) {
                    throw new \Exception("Sale of {$qty} \"{$item->name}\" will reduce stock below minimum ({$item->min_stock}).");
                }
            ...
            // SAVE PAYMENTS
            // **********************************
            $paymentSave = $this->saveSaleOrderPayments($request);

            if (!$paymentSave['status']) {
                throw new \Exception($paymentSave['message']);
            }
