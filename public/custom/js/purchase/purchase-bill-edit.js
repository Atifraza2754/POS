...
"use strict";
    ...
    // Add edit mode flag
    let isEditMode = (operation === 'update' || operation === 'convert');
    ...
    function setBottomInvoiceTotal(){
        ...
        // Only auto-fill payment if NOT in edit mode
        if (!isEditMode) {
            autoFillPaymentInputBox();
        } else {
            // In edit mode, recalculate final total based on existing discount and bilty values
            recalculateFinalTotalInEditMode(sumOfTotalColumn);
        }
    }

    /**
     * Recalculate Final Total in Edit Mode
     * This preserves the loaded discount and bilty values
     */
    function recalculateFinalTotalInEditMode(grandTotal) {
        const discountInput = document.querySelector('input[name="total_discount"]');
        const biltyInput = document.querySelector('input[name="bilty"]');
        const finalTotalInput = document.querySelector('input[name="final_total"]');
        const sumOfTotalCell = document.querySelector('.sum_of_total');
        
        if (discountInput && biltyInput && finalTotalInput) {
            const discountPercentage = parseFloat(discountInput.value) || 0;
            const biltyAmount = parseFloat(biltyInput.value) || 0;
    ...
    */
    ...
    /**
     * NEW DISCOUNT & BILTY CALCULATION SECTION
     * This section handles the final total calculation with discount and bilty
     */
    document.addEventListener("DOMContentLoaded", function () {
        const discountInput = document.querySelector('input[name="total_discount"]');
        const biltyInput = document.querySelector('input[name="bilty"]');
        ...
        function calculateFinalAmount() {
            ...
            // Apply discount on (grand_total + bilty)
            const totalBeforeDiscount = grandTotal + biltyAmount;
        ...
        // Listeners - only trigger on user input, not on page load
        if (discountInput) discountInput.addEventListener("input", calculateFinalAmount);
        if (biltyInput) biltyInput.addEventListener("input", calculateFinalAmount);
        if (grandTotalInput) grandTotalInput.addEventListener("input", calculateFinalAmount);

        // DON'T auto-calculate on page load if in edit mode
        // The values are already loaded from database
        if (!isEditMode) {
            calculateFinalAmount();
        }
    });