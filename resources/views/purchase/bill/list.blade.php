...
<script>
     ...
     $(document).on('change', '.purchase-switch', function () {

    if (!this.checked) {
        return; // ❌ ignore OFF
    }

    // turn off other switches (radio behavior)
    // $('.purchase-switch').not(this).prop('checked', false);

    let purchaseId = $(this).data('purchase-id');

    console.log('Selected Purchase ID:', purchaseId);

    if (!confirm('Are you sure you want to return this purchase?')) {
        $(this).prop('checked', false);
        return;
    }

    $.ajax({
        url: "{{ route('purchase.return.direct') }}",
        method: "POST",
        data: {
